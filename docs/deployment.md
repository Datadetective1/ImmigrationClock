...
> Protect `main` with a **ruleset** requiring the `verify` job — and put the
> GitHub Actions bot on that ruleset's bypass list, or the refresh workflows
> cannot deploy. See "Automated pushes to a protected `main`" below.
...
The production flow is deliberately direct — **scheduled job → regenerate →
commit → push to `main` → Vercel deploys**. No bot branch, no pull request, no
approval step. Data freshness should not wait on a human being awake.

That only works if the bot can write to `main`. On 2026-08-04 both refresh
workflows failed at their commit step, having quietly succeeded for weeks by
taking the "no change" path: classic branch protection rejects a direct push
from anyone, including `github-actions[bot]`, and **classic protection has no
bypass list**. Only rulesets do.

**Required repository setting:**

1. Settings → Rules → **Rulesets** → *New branch ruleset*, targeting `main`.
2. Add the restrictions you want to apply to *people* — typically *Require a
   pull request before merging* and *Require status checks to pass* → `verify`.
3. In **Bypass list**, add **the GitHub Actions bot** (listed as an app or
   role). This is the step that lets `GITHUB_TOKEN` push directly.
4. Settings → Branches → delete the old **classic** protection rule on `main`.
   A classic rule keeps applying even when a ruleset grants a bypass.

If the Actions entry is unavailable in the bypass picker, the fallback is a
GitHub App token (`actions/create-github-app-token`) with the app on the bypass
list. A personal access token also works but ties production deploys to one
person's account, so prefer the app.

**How the workflows push.** Both call
`.github/scripts/commit-and-push.sh`, which exists so the two cannot drift
apart — they previously kept separate hand-written `git add` lists, and
`src/lib/generated/warn-summary.json` was regenerated by both and committed by
neither, leaving the repo copy permanently stale. The script:

- stages the whole generated surface by directory (`src/lib/generated`,
  `public/api`, `public/data-manifest.json`, `public/pulse-email.*`), so a new
  output file is committed automatically rather than being forgotten,
- exits 0 without committing when nothing tracked actually changed,
- runs `git pull --rebase origin main` immediately before **every** push
  attempt, retrying a bounded number of times (`PUSH_ATTEMPTS`, default 5),
- **never force-pushes** — a conflict is someone else's commit, and resolving it
  by overwriting would destroy whatever landed first,
- fails loudly, naming the bypass list as the likely cause, when the budget runs
  out.

Both workflows share the concurrency group **`main-writer`** with
`cancel-in-progress: false`, so two refreshes can never interleave their
commits, and a half-finished refresh is never killed mid-flight.

**No `[skip ci]` in the bot's commit message.** It used to carry one. Vercel
treats `[skip ci]` as *do not deploy*, so the marker suppressed the production
deploy the whole workflow exists to trigger. Letting `ci.yml` run on the data
commit is also worth the minutes: the payload-budget and evidence-integrity
suites then actually see the new data.
